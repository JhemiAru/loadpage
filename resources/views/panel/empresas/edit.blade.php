    ...
    <form method="POST" action="{{ route('actualizarEmpresa', $empresa) }}" enctype="multipart/form-data" style="padding: 28px;">
        ...
        <div class="row g-4">
            ...
            <div class="col-md-6 form-group">
                ...
                <input type="text" id="nombre" name="nombre" class="form-control-panel @error('nombre') is-invalid @enderror" value="{{ old('nombre', $empresa->nombre) }}" required maxlength="255">
            ...
            <div class="col-md-6 form-group">
                ...
                @error('ciudad_id')<p class="field-error">{{ $message }}</p>@enderror
            </div>

            {{-- Prioridad --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="prioridad">Prioridad (menor = más arriba)</label>
                <input type="number" id="prioridad" name="prioridad" class="form-control-panel" value="{{ old('prioridad', $empresa->prioridad) }}" min="0">
            </div>

            {{-- Descripción (requerido) --}}
            <div class="col-12 form-group">
                <label class="form-label-panel" for="descripcion">Descripción <span class="text-danger">*</span></label>
                <textarea id="descripcion" name="descripcion" rows="3" class="form-control-panel @error('descripcion') is-invalid @enderror" required>{{ old('descripcion', $empresa->descripcion) }}</textarea>
            ...
            <div class="col-md-8 form-group">
                ...
                <input type="text" id="direccion" name="direccion" class="form-control-panel @error('direccion') is-invalid @enderror" value="{{ old('direccion', $empresa->direccion) }}" required>
            ...
            <div class="col-md-4 form-group">
                ...
                <input type="text" id="horario" name="horario" class="form-control-panel @error('horario') is-invalid @enderror" value="{{ old('horario', $empresa->horario) }}" required>
                @error('horario')<p class="field-error">{{ $message }}</p>@enderror
            </div>

            {{-- Teléfono --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="telefono">Teléfono</label>
                <input type="text" id="telefono" name="telefono" class="form-control-panel" value="{{ old('telefono', $empresa->telefono) }}">
            </div>

            {{-- Email --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="email">Email</label>
                <input type="email" id="email" name="email" class="form-control-panel" value="{{ old('email', $empresa->email) }}">
            </div>

            {{-- Web y redes --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="web">Sitio web</label>
                <input type="url" id="web" name="web" class="form-control-panel" value="{{ old('web', $empresa->web) }}">
            </div>
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="facebook">Facebook</label>
                <input type="url" id="facebook" name="facebook" class="form-control-panel" value="{{ old('facebook', $empresa->facebook) }}">
            </div>

            {{-- Promoción y descuento --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="promocion">Promoción</label>
                <input type="text" id="promocion" name="promocion" class="form-control-panel" value="{{ old('promocion', $empresa->promocion) }}">
            </div>
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="descuento">Descuento</label>
                <input type="text" id="descuento" name="descuento" class="form-control-panel" value="{{ old('descuento', $empresa->descuento) }}">
            </div>

            {{-- Videos --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="video">Código de video (embed)</label>
                <input type="text" id="video" name="video" class="form-control-panel" value="{{ old('video', $empresa->video) }}">
            </div>
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="videof">Video destacado (URL)</label>
                <input type="url" id="videof" name="videof" class="form-control-panel" value="{{ old('videof', $empresa->videof) }}">
            </div>
            ...
            <div class="col-12 form-group">
                ...
                @error('imagen')<p class="field-error">{{ $message }}</p>@enderror
            </div>
            ...
            <div class="col-12 form-group">
                ...
                @error('imagen1')<p class="field-error">{{ $message }}</p>@enderror
            </div>

            {{-- Coordenadas y mapa --}}
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="latitud">Latitud</label>
                <input type="number" step="any" id="latitud" name="latitud" class="form-control-panel" value="{{ old('latitud', $empresa->latitud) }}">
            </div>
            <div class="col-md-6 form-group">
                <label class="form-label-panel" for="longitud">Longitud</label>
                <input type="number" step="any" id="longitud" name="longitud" class="form-control-panel" value="{{ old('longitud', $empresa->longitud) }}">
            </div>
            <div class="col-12 form-group">
                <label class="form-label-panel" for="mapa">Código de mapa (iframe)</label>
                <textarea id="mapa" name="mapa" rows="2" class="form-control-panel">{{ old('mapa', $empresa->mapa) }}</textarea>
            </div>
        </div>
